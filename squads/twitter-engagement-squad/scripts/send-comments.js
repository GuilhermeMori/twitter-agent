...
function createWordDocument(checks, rawPostsData) {
  let wordContent = `
    ...
    <body>
      <h1>Twitter Engagement Squad - Comentários Aprovados</h1>
      <p><strong>Data:</strong> ${new Date().toLocaleDateString('pt-BR')}</p>
      <p><strong>Total de comentários:</strong> ${checks.length}</p>
      <hr>
  `;
    ...
    wordContent += `
      <div class="post">
        <div class="post-header">
          ...
          <p><strong>Engajamento:</strong> ${originalPost ? `${originalPost.likes} likes, ${originalPost.reposts} reposts` : 'N/A'}</p>
        </div>
  ...
  try {
    ...
    let emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; padding: 20px;">
        <h2 style="color: #1DA1F2;">🐦 Twitter Engagement Squad - Aprovação</h2>
        <p>Olá! O ciclo de hoje foi concluído com sucesso.</p>
        <p><strong>Total de comentários aprovados:</strong> ${reviewedData.checks.length}</p>
        <p><strong>Score médio:</strong> ${reviewedData.summary.average_score}/10</p>
        <hr style="border: 0; border-top: 1px solid #eee; margin: 20px 0;">
        <h3>Top 3 Comentários:</h3>
    `;
    ...
    if (remaining.length > 0) {
      emailHtml += `
        <div style="background: #fff3cd; border: 1px solid #ffc107; padding: 15px; border-radius: 8px; margin-top: 20px;">
          <p style="margin: 0;"><strong>📎 Anexo:</strong> Os demais ${remaining.length} comentários estão no arquivo Word anexo.</p>
        </div>
      `;
    }
    ...
    emailHtml += `
        ...
        <ul style="line-height: 1.8;">
          <li>Revise os comentários acima e no anexo</li>
          <li>Responda este e-mail com sua aprovação ou ajustes</li>
          <li>Ou acesse o dashboard para gerenciar</li>
        </ul>
        <small style="color: #999;">Gbm Tecnologia | Automação Estratégica com IA</small>
      </div>
    `;
    ...
    const mailOptions = {
      ...
      subject: `🚀 Twitter Engagement: ${reviewedData.checks.length} Comentários Aprovados`,
      html: emailHtml,
      attachments: [
        {
