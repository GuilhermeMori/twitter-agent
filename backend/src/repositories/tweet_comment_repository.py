...
class TweetCommentRepository:
    ...
    def create(self, comment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "Created tweet comment: %s for tweet %s",
                result.data[0]["id"],
                comment_data["tweet_id"],
            )
            return result.data[0]

        except Exception as e:
            logger.error(
                "Failed to create tweet comment for tweet %s: %s",
                comment_data.get("tweet_id"),
                str(e),
            )
            raise
    ...
    def get_by_campaign_and_tweet(
        self, campaign_id: str, tweet_id: str
        ...
        try:
            ...
            logger.error(
                "Failed to retrieve comment for tweet %s in campaign %s: %s",
                tweet_id,
                campaign_id,
                str(e),
            )
    ...
    def mark_as_regenerated(self, campaign_id: str, tweet_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to mark comments as regenerated for tweet %s: %s", tweet_id, str(e)
            )
            raise
    ...
    def get_campaign_stats(self, campaign_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_chars = (
                sum(c["char_count"] for c in valid_comments) / len(valid_comments)
                if valid_comments
                else 0
            )
    ...
    def delete_by_campaign(self, campaign_id: str) -> int:
        ...
        try:
            result = (
                self._db.table("tweet_comments").delete().eq("campaign_id", campaign_id).execute()
            )
