...
class TweetAnalysisRepository:
    ...
    def create(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "Created tweet analysis: %s for tweet %s",
                result.data[0]["id"],
                analysis_data["tweet_id"],
            )
            return result.data[0]

        except Exception as e:
            logger.error(
                "Failed to create tweet analysis for tweet %s: %s",
    ...
    def get_by_campaign_and_tweet(
        ...
        try:
            ...
            logger.error(
                "Failed to retrieve analysis for tweet %s in campaign %s: %s",
                tweet_id,
                campaign_id,
                str(e),
            )
    ...
    def mark_top_tweets(self, campaign_id: str, top_n: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self._db.table("tweet_analysis").update({"is_top_3": False}).eq(
    ...
    def delete_by_campaign(self, campaign_id: str) -> int:
        ...
        try:
            result = (
                self._db.table("tweet_analysis").delete().eq("campaign_id", campaign_id).execute()
            )
