...
password_strategy = st.text(
    alphabet=st.characters(whitelist_categories=("Lu", "Ll", "Nd", "P")), min_size=8, max_size=50
)
...
def test_property_3_configuration_validation_rejects_incomplete_data(
    ...
    **Validates: Requirements 1.2**
    ...
    has_empty_field = (
        not apify_token.strip() or not openai_token.strip() or not smtp_password.strip()
    )
