...
from src.models.campaign import ScrapingConfig, SearchType, Tweet, Campaign, CampaignConfig, CampaignStatus
...
def analysis_strategy(draw):
    ...
    return Analysis(
        ...
        recommendations=draw(st.lists(xml_safe_text(min_size=10, max_size=200), min_size=1, max_size=5))
    )
...
def test_property_18_local_filtering_enforces_engagement_criteria(raw_tweets, config):
    ...
    **Validates: Requirements 5.11**
    ...
    for tweet in raw_tweets:
        ...
        meets_criteria = (
            likes >= config.min_likes and
            retweets >= config.min_retweets and
    ...
    **Validates: Requirements 5.12**
    ...
    for tweet in transformed:
        ...
        assert isinstance(tweet, Tweet), \
            "Transformed tweet should be a Tweet model instance"
        
        # Check all required fields are present and not None
        for field in required_fields:
            assert hasattr(tweet, field), \
                f"Transformed tweet missing required field: {field}"
...
def test_property_20_document_contains_all_required_sections(campaign, tweets, analysis):
    ...
    **Validates: Requirements 7.3, 7.4**
    ...
    try:
        ...
        assert campaign.name in full_text, \
            "Document should contain campaign name"
        
        # Verify execution date is present (header section)
        assert "Generated:" in full_text, \
            "Document should contain generation date"
        
        # Verify status is present (header section)
        assert "Status:" in full_text, \
            "Document should contain campaign status"
        
        # Verify configuration section is present
        assert "Configuration Used" in full_text, \
            "Document should contain 'Configuration Used' section"
        
        # Verify configuration parameters are present
        assert "Search Type" in full_text, \
            "Document should contain search type"
        assert "Language" in full_text, \
            "Document should contain language"
        assert "Engagement Filters" in full_text or "min_likes" in full_text.lower(), \
            "Document should contain engagement filters"
        assert "Time Window" in full_text or "hours" in full_text.lower(), \
            "Document should contain time window"
        
        # Verify tweets section is present
        assert "Collected Tweets" in full_text, \
            "Document should contain 'Collected Tweets' section"
        ...
        assert tweet_content_found, \
            "Document should contain tweet content"
        
        # Verify analysis section is present
        assert "Analysis" in full_text, \
            "Document should contain 'Analysis' section"
        
        # Verify analysis subsections are present
        assert "Summary" in full_text, \
            "Document should contain analysis summary"
        assert "Sentiment" in full_text, \
            "Document should contain sentiment analysis"
        
        # Verify analysis content is present
        assert analysis.summary in full_text or analysis.sentiment in full_text, \
...
def test_filtering_preserves_order(raw_tweets, config):
    ...
    for i in range(len(filtered) - 1):
        ...
        assert current_idx < next_idx, \
            "Filtering should preserve the original order of tweets"
