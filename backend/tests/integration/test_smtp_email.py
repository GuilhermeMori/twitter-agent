...
def email_service():
    ...
    return EmailService(sender_email="sender@gmail.com", smtp_password="app_password_123")
...
def test_complete_email_sending_workflow(
    mock_exists, mock_file, mock_smtp, email_service, completed_campaign
):
    ...
    email_service.send_campaign_results(
        "recipient@example.com", completed_campaign, "/tmp/results.docx"
    )
...
@patch("builtins.open", new_callable=mock_open, read_data=b"fake docx content")
@patch("os.path.exists")
def test_email_body_content(mock_exists, mock_file, mock_smtp, email_service, completed_campaign):
...
def test_email_attachment_handling(
    ...
    email_service.send_campaign_results(
        "recipient@example.com", completed_campaign, "/tmp/campaign_results.docx"
    )
...
@patch("builtins.open", new_callable=mock_open, read_data=b"fake docx content")
@patch("os.path.exists")
def test_smtp_send_failure(mock_exists, mock_file, mock_smtp, email_service, completed_campaign):
...
@patch("smtplib.SMTP")
@patch("os.path.exists")
def test_missing_attachment_file(mock_exists, mock_smtp, email_service, completed_campaign):
    ...
    email_service.send_campaign_results(
        "recipient@example.com", completed_campaign, "/tmp/nonexistent.docx"
    )
...
@patch("builtins.open", new_callable=mock_open, read_data=b"fake docx content")
@patch("os.path.exists")
def test_smtp_timeout_error(mock_exists, mock_file, mock_smtp, email_service, completed_campaign):
...
def test_invalid_recipient_email(
    ...
    with pytest.raises(Exception):
        email_service.send_campaign_results("invalid@", completed_campaign, "/tmp/results.docx")
...
@patch("builtins.open", new_callable=mock_open, read_data=b"fake docx content")
@patch("os.path.exists")
def test_uses_tls_encryption(mock_exists, mock_file, mock_smtp, email_service, completed_campaign):
...
def test_send_to_multiple_recipients(
    ...
    for recipient in recipients:
        email_service.send_campaign_results(recipient, completed_campaign, "/tmp/results.docx")
