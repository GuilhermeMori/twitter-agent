...
def test_list_campaigns_with_pagination(campaign_repository, mock_supabase_client, realistic_campaign_data):
...
def test_get_campaign_results(campaign_repository, mock_supabase_client, realistic_tweet_results):
    ...
    mock_table.select.return_value.eq.return_value.order.assert_called_once_with("timestamp", desc=True)
...
def test_create_configuration(configuration_repository, mock_supabase_client, realistic_configuration_data):
...
def test_get_configuration(configuration_repository, mock_supabase_client, realistic_configuration_data):
...
def test_get_configuration_by_email(configuration_repository, mock_supabase_client, realistic_configuration_data):
...
def test_update_configuration(configuration_repository, mock_supabase_client):
    ...
    config_id = str(uuid4())
    updates = {
        "apify_token": "new_encrypted_token",
        "openai_token": "new_encrypted_openai_token"
    }
