...
def test_complete_upload_workflow(
    mock_exists, mock_file, storage_service, mock_supabase_client, campaign_id
):
...
def test_generate_signed_url_with_custom_expiry(storage_service, mock_supabase_client, campaign_id):
    ...
    mock_bucket.create_signed_url.return_value = {"signedURL": "https://storage.example.com/signed"}
...
def test_generate_signed_url_alternative_response_format(
    storage_service, mock_supabase_client, campaign_id
):
    ...
    mock_bucket.create_signed_url.return_value = {"signedUrl": expected_url}  # lowercase 'url'
...
def test_delete_document_handles_permission_error(
    storage_service, mock_supabase_client, campaign_id
):
...
@patch("builtins.open", new_callable=mock_open, read_data=b"fake docx content")
@patch("os.path.exists")
def test_upload_sets_correct_content_type(
    mock_exists, mock_file, storage_service, mock_supabase_client, campaign_id
):
