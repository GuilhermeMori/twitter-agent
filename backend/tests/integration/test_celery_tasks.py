...
@patch("src.workers.campaign_executor.CampaignRepository")
@patch("src.workers.campaign_executor.get_supabase_client")
def test_campaign_not_found_error(mock_get_db, mock_campaign_repo_class, campaign_id):
...
def test_scraping_failure_updates_status(
    ...
    failed_update = [
        call for call in mock_repo.update_status.call_args_list if call[0][1] == "failed"
    ]
...
def test_email_failure_updates_status(
    ...
    mock_email_service = Mock()
    mock_email_service_class.return_value = mock_email_service
    mock_email_service.send_campaign_results.side_effect = RuntimeError(
