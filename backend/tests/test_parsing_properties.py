...
campaign_config_strategy = st.builds(
    CampaignConfig,
    profiles=st.one_of(st.none(), st.lists(username_strategy, min_size=1, max_size=5)),
    keywords=st.one_of(st.none(), st.lists(keyword_strategy, min_size=1, max_size=5)),
...
def test_property_7_profile_parsing_removes_at_symbol(username):
    ...
    **Validates: Requirements 3.4, 20.7**
    ...
    assert (
        parsed_with_at[0] == username
    ), f"Should remove @ symbol: expected '{username}', got '{parsed_with_at[0]}'"
    ...
    assert (
        parsed_without_at[0] == username
    ), f"Should preserve username: expected '{username}', got '{parsed_without_at[0]}'"

    # Both should produce the same result
    assert (
        parsed_with_at == parsed_without_at
    ), "Parsing with or without @ should produce same result"
...
def test_property_8_keyword_preservation(keyword):
    ...
    **Validates: Requirements 3.5, 20.8**
    ...
    assert (
        parsed[0] == expected
    ), f"Should preserve keyword exactly: expected '{expected}', got '{parsed[0]}'"
...
def test_property_10_list_parsing_handles_multiple_delimiters(items):
    ...
    **Validates: Requirements 20.1, 20.2, 20.3, 20.4, 20.5, 20.6**
    ...
    assert (
        parsed_comma == items
    ), f"Should correctly parse comma-separated: expected {items}, got {parsed_comma}"
    ...
    assert (
        parsed_newline == items
    ), f"Should correctly parse newline-separated: expected {items}, got {parsed_newline}"
    ...
    assert (
        parsed_mixed == items
    ), f"Should correctly parse mixed delimiters: expected {items}, got {parsed_mixed}"
    ...
    assert (
        parsed_whitespace == items
    ), f"Should correctly trim whitespace: expected {items}, got {parsed_whitespace}"
...
def test_property_11_parsed_list_is_never_empty_after_valid_input(input_string):
    ...
    **Validates: Requirements 20.9**
    ...
    assert (
        len(parsed_profiles) > 0
    ), f"Should produce non-empty list for profiles: input='{input_string}'"

    # Parse as keywords
    parsed_keywords = parser.parse_keywords(input_string)
    assert (
        len(parsed_keywords) > 0
    ), f"Should produce non-empty list for keywords: input='{input_string}'"
...
def test_property_21_configuration_formatting_is_reversible(config):
    ...
    **Validates: Requirements 22.1**
    ...
    if config.profiles:
        ...
        assert (
            reparsed_profiles == config.profiles
    ...
    if config.keywords:
        ...
        # Should match the original
        assert (
            reparsed_keywords == config.keywords
...
def test_empty_input_produces_empty_list(empty_input):
    ...
    assert (
        len(parsed_profiles) == 0
    ), f"Empty input should produce empty list for profiles: input='{empty_input}'"

    # Parse as keywords
    parsed_keywords = parser.parse_keywords(empty_input)
    assert (
        len(parsed_keywords) == 0
    ), f"Empty input should produce empty list for keywords: input='{empty_input}'"
...
def test_multiple_at_symbols_are_stripped(username, at_count):
    ...
    assert (
        parsed[0] == username
    ), f"Should remove all @ symbols: expected '{username}', got '{parsed[0]}'"
...
def test_formatting_adds_at_prefix(profiles):
    ...
    assert (
        formatted == expected
    ), f"Should format as comma-separated with @ prefix: expected '{expected}', got '{formatted}'"
