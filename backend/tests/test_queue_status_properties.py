...
def test_property_23_fifo_queue_processing_order(campaign_names):
    ...
    **Validates: Requirements 4.2**
    ...
    with patch('src.workers.campaign_executor.execute_campaign', create=True) as mock_task:
        ...
        assert len(retrieved_names) == len(expected_names), \
            "Should retrieve all created campaigns"
    ...
    **Validates: Requirements 4.3, 4.7, 4.12**
    ...
    if is_valid_transition:
        ...
        try:
            ...
            assert updates["status"] == target_status.value, \
                f"Status should be updated to {target_status.value}"
            
        except Exception as e:
            pytest.fail(f"Valid transition {initial_status.value} → {target_status.value} should not raise exception: {e}")
    else:
        # Invalid transition - verify it's not in the valid transitions list
        assert target_status not in valid_transitions.get(initial_status, []), \
...
def test_terminal_states_cannot_transition(terminal_status, target_status):
    ...
    assert len(valid_transitions[terminal_status]) == 0, \
...
def test_running_state_must_come_from_pending(campaign_name):
    ...
    with patch('src.workers.campaign_executor.execute_campaign', create=True) as mock_task:
        ...
        assert created_record["status"] == CampaignStatus.PENDING.value, \
        ...
        assert valid_first_transition == CampaignStatus.RUNNING.value, \
