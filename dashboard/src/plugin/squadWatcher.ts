  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        // Twitter Squad Specific Endpoints
        if (url === "/api/squad/twitter/config" && req.method === "GET") {
          const configPath = path.join(squadsDir, "twitter-engagement-squad", "output", "research-focus.md");
          try {
        ...
        if (url === "/api/squad/twitter/config" && req.method === "POST") {
          let body = "";
          ...
          req.on("end", async () => {
            try {
              ...
              res.writeHead(500);
              res.end("Error saving config");
            }
          });
          return;
        }

        if (url === "/api/squad/twitter/history" && req.method === "GET") {
          const historyPath = path.join(squadsDir, "twitter-engagement-squad", "output", "reviewed-comments.md");
          try {
            const content = await fsp.readFile(historyPath, "utf-8");
            res.setHeader("Content-Type", "application/json");
            res.end(JSON.stringify({ content }));
          } catch {
            res.end(JSON.stringify({ content: "status: No history yet" }));
          }
          return;
        }

        if (url === "/api/squad/twitter/results" && req.method === "GET") {
          const resultsPath = path.join(squadsDir, "twitter-engagement-squad", "output", "raw-posts.md");
          try {
            const content = await fsp.readFile(resultsPath, "utf-8");
            res.setHeader("Content-Type", "application/json");
            res.end(JSON.stringify({ content }));
          } catch {
            res.end(JSON.stringify({ content: "posts: []" }));
          }
          return;
        }

        if (url === "/api/squad/twitter/run" && req.method === "POST") {
          // Criar um arquivo de trigger que o IDE pode monitorar
          const triggerPath = path.join(squadsDir, "twitter-engagement-squad", ".run-trigger");
          const triggerData = {
            timestamp: new Date().toISOString(),
            source: "dashboard",
            command: "/opensquad run twitter-engagement-squad"
          };
          
          try {
            await fsp.writeFile(triggerPath, JSON.stringify(triggerData, null, 2), "utf-8");
            console.log(`[squad-watcher] Run trigger created at ${triggerPath}`);
            
            res.setHeader("Content-Type", "application/json");
            res.end(JSON.stringify({ 
              status: "triggered",
              message: "Squad execution triggered. Check your IDE for the /opensquad runner.",
              triggerFile: triggerPath
            }));
          } catch (error) {
            console.error(`[squad-watcher] Failed to create trigger:`, error);
            res.writeHead(500);
            res.end(JSON.stringify({ error: "Failed to trigger squad execution" }));
          }
          return;
        }
